...
public class StaffRecordDeletion extends JFrame{
    
    MaintainStaff staffControl;
    
    MaintainLogin loginControl;
    
    private char accType;
    
    private Staff staffDel;
    
    private Authentication authIn;
    
    //declarations
    private JLabel lb_Title = new JLabel("Staff Record Deletion",SwingConstants.CENTER);
    private JLabel lb_instruction = new JLabel("Please enter the Staff ID below to delete the record of the staff.",SwingConstants.CENTER);
...
//    private JTextField jtf_emp_stat = new JTextField(2);
   ...
   public void paintComponent(Graphics g) {  
     ...
     StaffRecordDeletion.class.getResource("../images/green1.jpg"));  
     g.drawImage(img1, 0, 0, this.getWidth(), this.getHeight(), this);  
     }};
    
    private JPanel big_container = new JPanel(new GridLayout(2,2,0,0));
    private JPanel groupA = new JPanel(new GridLayout(6,1,0,0));//8100,9100 10,1,0,0
    private JPanel groupB = new JPanel(new GridLayout(6,1,0,0));//7100 8100
    private JPanel groupC = new JPanel(new GridLayout(5,1,0,0));//4100
    private JPanel groupD = new JPanel(new GridLayout(5,1,0,0));//9100 10100 8100
    ...
    public StaffRecordDeletion(){
        
    }
    
    public StaffRecordDeletion(char accType){
...
//        jtf_emp_stat.setEditable(false);
        ...
        //set button colors
        jbtDelete.setBackground(jbtColor);
...
//        groupD.add(groupD_r8);
        ...
        getRootPane().setDefaultButton(jbtDelete);
        
        setIcon();
        setTitle("Staff Record Deletion");
        setSize(900,705);
    ...
    private class ButtonListener implements ActionListener{
        public void actionPerformed(ActionEvent e){
            if(e.getSource() == jbtDelete){
                    ...
                    if(staffID.equals("")){
                        JOptionPane.showMessageDialog(null,"Please enter a staff ID to delete record.","Empty staff ID",JOptionPane.WARNING_MESSAGE);
                        jtfStaffID.requestFocusInWindow();
                    }
                    else{
                        //temporary assignment
                        //staffDel = null;
                        staffDel = staffControl.getRecord(staffID);

                            if(staffDel != null){
                                jtf_fname.setText(staffDel.getFirstname());
                                jtf_lname.setText(staffDel.getLastname());
                                jtf_gender.setText(String.valueOf(staffDel.getGender()));
                                //split string for IC
...
//                                jtf_bankAccNo.setText(staff.getBankAccNo());
                                jtf_street_add1.setText(staffDel.getStreetaddress1());
                                jtf_street_add2.setText(staffDel.getStreetaddress2());
                                jtf_city.setText(staffDel.getCity());
                                //jtf_postcode.setText(String.valueOf(staff.getPostcode())); //manual conversion using static method
                                jtf_state.setText(staffDel.getState());
                                //jtf_country.setText(staff.getCountry());
                                String mobileNo = staffDel.getMobilephoneno();
                                ...
                                try{
                                    String homeNo = staffDel.getHomephoneno();
                                ...
                                jtf_email.setText(staffDel.getEmailaddress());
                                jtf_staffID.setText(staffDel.getStaffid());
                                jtf_position.setText(staffDel.getPosition());
                                //salary can be directly read using getString despite its double data type in db
                                jtf_salary.setText(String.valueOf(staffDel.getSalary())); //conversion using static method
...
//                                jtf_emp_stat.setText(staff.getEmpStat());
                                ...
                                int confirmDel = 0;
                                confirmDel = JOptionPane.showConfirmDialog(null, "Are you sure want to delete the record of\n Staff ID: " + jtf_staffID.getText(), "Confirm?", JOptionPane.YES_NO_OPTION);
                                ...
                                if(confirmDel == JOptionPane.YES_OPTION){
                                    ...
                                    int delStaffResult = -1;
                                    
                                    authIn = new Authentication();
                                    
                                    authIn.setStaffid(staffID);
                                    
                                    accExist = loginControl.verifyExistence(authIn);
                                    
                                    if(accExist == 'Y'){ //There is an account attached to the staff to be deleted
                                        delAccResult = loginControl.deleteAccount(authIn);
                                        
                                        if(delAccResult > 0){ //account in Authentication has been deleted successfully
                                            delStaffResult = staffControl.deleteRecord(staffDel);
                                            
                                            if(delStaffResult > 0){  //delete account + staff successful
                                                JOptionPane.showMessageDialog(null,"Record for staff ID: "+ staffID + " has been deleted successfully.","Staff Record Deleted",JOptionPane.INFORMATION_MESSAGE);
                                                resetPage();
                                            }
                                            else{ //delete account successful but delete staff failed
                                                JOptionPane.showMessageDialog(null,"Some error(s) has occured when deleting the record of\n Staff ID: " + staffID,"Staff Deletion Failed",JOptionPane.ERROR_MESSAGE);
                                            }
                                            
                                        }
                                        else{ //account in Authentication has been delete failed
                                            JOptionPane.showMessageDialog(null, "The deletion of the account\n attached with Staff ID: " + staffID + " has failed.","Account Deletion Failed", JOptionPane.ERROR_MESSAGE);
                                        }
                                    }
                                    else{ //No account attached with the staff to be deleted
                                        
                                        delStaffResult = staffControl.deleteRecord(staffDel);
                                            
                                        if(delStaffResult > 0){  //delete staff successful
                                            JOptionPane.showMessageDialog(null,"Record for staff ID: "+ staffID + " has been deleted successfully.","Staff Record Deleted",JOptionPane.INFORMATION_MESSAGE);
                                            resetPage();
                                        }
                                        else{ //delete staff failed
                                            JOptionPane.showMessageDialog(null,"Some error(s) has occured when deleting the record of\n Staff ID: " + staffID,"Staff Deletion Failed",JOptionPane.ERROR_MESSAGE);
                                        }
                                    }
                            ...
                            //staffControl.deleteRecord(jtf_staffID.getText());
                            
                            //dispose and call back submenu
